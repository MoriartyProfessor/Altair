...
namespace Rays
{
    ...
    template<>
    BitBoard gen_diagonals<SOUTH_WEST>(Square square)
    {
    ...
    template<>
    BitBoard gen_diagonals<SOUTH_EAST>(Square square)
    {
    ...
    template<>
    BitBoard gen_diagonals<NORTH_EAST>(Square square)
    {
    ...
    void init()
    {
        ...
        for(Square square = SQ_A1; square < N_SQUARES; ++square)
        {
            ...
            rays[NORTH_WEST][square] = gen_diagonals<NORTH_WEST>(square);

            rays[SOUTH_WEST][square] = gen_diagonals<SOUTH_WEST>(square);

            rays[SOUTH_EAST][square] = gen_diagonals<SOUTH_EAST>(square);

            rays[NORTH_EAST][square] = gen_diagonals<NORTH_EAST>(square);
        }
    }
