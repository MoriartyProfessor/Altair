...
namespace Patterns
{
    BitBoard pawn_attacks_[N_COLORS][N_SQUARES];
    BitBoard knight_attacks_[N_SQUARES];
    BitBoard king_attacks_[N_SQUARES];

    BitBoard gen_pawn_attacks_(Square square, Color color)
    {
        BitBoard pawn_bitboard = BitBoards::square_set_in(square);
        BitBoard pawn_attacks = EMPTY_BB;

        if(color == WHITE)
        {
            pawn_attacks |= BitBoards::step<NORTH_EAST>(pawn_bitboard);
            pawn_attacks |= BitBoards::step<NORTH_WEST>(pawn_bitboard);
        }
        else
        {
            pawn_attacks |= BitBoards::step<SOUTH_EAST>(pawn_bitboard);
            pawn_attacks |= BitBoards::step<SOUTH_WEST>(pawn_bitboard);
        }
    ...
    void init_pawn_attacks_()
    {
        for(Square square = SQ_A1; square < N_SQUARES; ++square)
        {
            pawn_attacks_[WHITE][square] = gen_pawn_attacks_(square, WHITE);
            pawn_attacks_[BLACK][square] = gen_pawn_attacks_(square, BLACK);
        }
    }
